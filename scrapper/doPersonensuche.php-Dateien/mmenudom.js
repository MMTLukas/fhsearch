...
Version 5.760 - Built: Thursday October 12 2006 - 10:25
...
function o$() {
	...
	if (_M[7] || !_startM || (op5 || op6) || _W.buildAllMenus) {
		...
		m_e += _p5;
		_mt += "<" + m_e + " name=mM1 id=mmlink" + $m + " href=# onmouseout=hidetip() onclick='return $K(this._itemRef)' onmouseover='_p1(this);_mot=$P(_mot)' style='outline:none;line-height:normal;background:transparent;text-decoration:none;height:1px;width:1px;overflow:hidden;position:" + B$ + ";" + _fSz + "</" + m_e + ">"
	} else {
		if (_begn == 1) {
			for (_b = 0; _b < _M[0].length; _b++) {
				$q(_el);
...
function $V($m, _on) {
	...
	if (ie55) {
		if (_on) {
			...
			if (f) {
				$E(f, p[0], p[1], p[2], p[3]);
				f.style.visibility = $6
			}
			f.style.zIndex = _M[22].style.zIndex - 2;
			_M.ifr = f
		} else {
			var g = $c("iF" + ($mD - 1));
			if (g) {
